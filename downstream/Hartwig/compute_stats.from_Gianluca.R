...
MARKERS <- c("CtIP", "GRHL")
SOURCES <- c("ICGC", "HART")
N_DON <- setNames(list(214, 1059), SOURCES) # hart = n. of folders 
...
output_filename <- fs::path(path_results_plots, "RPlots.compute_stats.from_Gianluca.pdf")
pdf(output_filename)
...
for(marker in MARKERS){
  ...
  # ICGC
  overlaps$ICGC[[marker]] <- read_tsv(fs::path(path_overlaps_icgc, paste0("Table_enh_SSMs_", marker, ".all_overlaps.3kb_WIN.from_Gianluca.tsv"))) %>% 
    dplyr::select(c(1:13, 16:17, 14, 21)) 
